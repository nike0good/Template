...
struct Point
{
	...
	double sinAngle(const Point& p1, const Point& p2)const{
		Point t1 = *this - p1, t2 = *this - p2;
		return abs(t1 * t2) / sqrt(t1.r2()*t2.r2());
	}
	double tanAngle(const Point& o)const{
	...
	Point nearestPoint(const Point& o, double r)const;
};
...
int lineConvexIntersectPointInit(const Point* p, int n, double angle[])
{
	int ret = 0;
...
int lineConvexIntersect(const Point& p1, const Point& p2, const Point* p, int n, double angle[], int offset, int &ret1, int &ret2)
{
	int pos[2];
	double k[2];
...
void lineConvexIntersectPoint(const Point& p1, const Point& p2, const Point* p, int n, int i1, int i2, Point &ret1, Point &ret2)
{
...
bool lineSegInPolygon(Point p1, Point p2, const Point* p, int n)
{
	if (p2 < p1)swap(p1, p2);
...
{
	vector<pair<double, const Point*> > v(n), line(n);
	for (int i = 0; i < n; i++)
...
double convexDiameter(const Point* p, int n, Point& ret1, Point& ret2)
{
	double ret = 0;
...
// These points must be put counter - clockwise.
//Ensure p[n] exists and p[n]=p[0].
double convexWidth(const Point* p, int n)
{
	double ret = INF;
	for (int i = 0, j = 1; i < n; i++){
		double t1 = (p[i + 1] - p[i]) * (p[j] - p[i]), t2;
...
double circleCircleIntersectArea(const Point& o1, double r1, const Point& o2, double r2)
{
...
void circleCirclePointcutOuter(const Point& o1, double r1, const Point& o2, double r2, Point* ret)
{
	Point o12 = o2 - o1;
...
void circleCirclePointcutInner(const Point& o1, double r1, const Point& o2, double r2, Point* ret)
{
	Point o12 = o2 - o1;
	double d12 = o12.r2(), r = (r1 + r2) / d12;
	Point foot1 = o1 + o12*(r*r1), foot2 = o2 - o12*(r*r2);
...
Point Point::nearestPoint(const Point& o, double r)const
{
	Point p = *this - o;
	double d = p.r();
	if (EQ(d, 0))return o;
...
double circlePolygonAreaIntersect(const Point& o, double r, const Point* p, int n)
{
	double area = 0;
	Point p1, p2;
	for (int i = 0, j = n - 1; i < n; j = i++){
